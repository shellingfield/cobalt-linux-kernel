...
"eepro100.c:v0.38 2/17/98 Donald Becker linux-eepro100@cesdis.gsfc.nasa.gov\n";

static const char *cobalt_version =
"cobalte100.c:v0.01 4/8/98 David S. Miller (davem@dm.cobaltmicro.com)\n";
...
#ifdef CONFIG_COBALT_27
static int speedo_linkcheck(void *cookie);
#endif
...
int eepro100_init(struct device *dev)
{
	...
	if (pcibios_present()) {
		...
		for (; pci_index < 8; pci_index++) {
			...
			/* COBALT LOCAL: We know this will happen and don't need to see
			 *               the messages... -DaveM
			 */
						  ...
						  int card_idx)
{
...
#ifdef CONFIG_COBALT_27
		flush_cache_pre_dma_out((unsigned long)str, sizeof(s32) * 6);
		self_test_results = (s32*) ((((long) CACHED_TO_UNCACHED(str)) + 15) & ~0xf);
#else
	...
	flush_cache_pre_dma_out((unsigned long)dev->priv, sizeof(struct speedo_private));
...
#endif
#ifdef CONFIG_COBALT_27
	lcd_register_linkcheck_func(dev->name[3] - '0', speedo_linkcheck, dev);
#endif
...
speedo_open(struct device *dev)
{
	 ...
	 * handle them.   -DaveM
	 */
	if (request_irq(dev->irq, &speedo_interrupt, SA_INTERRUPT,
...
#ifdef CONFIG_COBALT_27
		/* COBALT LOCAL: We're not DMA coherent, so reference in uncached space.  -DaveM */
		flush_cache_pre_dma_out((unsigned long)rxf, sizeof(struct RxFD));
		rxf = (void *)CACHED_TO_UNCACHED(rxf);
#endif
		sp->rx_ringp[i] = rxf;
		if (last_rxf)
...
speedo_rx(struct device *dev)
{
	...
	while ((status = sp->rx_ringp[entry]->status) & RX_COMPLETE) {
		...
		if (status & 0x0200) {
			 ...
			 */
			flush_cache_post_dma_in((unsigned long)bus_to_virt(sp->rx_ringp[entry]->rx_buf_addr),
									pkt_len);
#endif
