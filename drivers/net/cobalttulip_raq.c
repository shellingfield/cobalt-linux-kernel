...
/* If another Tulip is in the expansion PCI slot, we want the on-board
 * Tulip to be found first, thus... -DaveM
 */
...
static void parse_eeprom(struct device *dev, int chip_id);
static int read_eeprom(int ioaddr, int location);
static int mdio_read(int ioaddr, int phy_id, int location);
#if 0
/* Keep this around, we may need it later.  -DaveM */
...
static void tulip_rx(struct device *dev);
...
#if 0
			/* XXX Hack hack hack -DaveM */
			if((pci_ioaddr & 0xf0000000) != 0x10000000 ||
			   (pci_ioaddr & 0xfffff000) == 0x10108000) {
					unsigned short pci_cmd;

					/* Expansion slot card... */
					pci_ioaddr = 0x10108000;
								   ...
								   int chip_id, int options)
{
...
#if defined(__mips__)
		/* XXX This is pretty gross, but for now... -DaveM */
...
#if defined(__mips__)
	{
			int dlen = ((sizeof(struct tulip_rx_desc) * RX_RING_SIZE) +
						(sizeof(struct tulip_tx_desc) * TX_RING_SIZE));
			unsigned long descs = (unsigned long) kmalloc(dlen + 32,
														  (GFP_KERNEL | GFP_DMA));

			descs = (descs + (32UL - 1UL)) & ~(32UL - 1UL);
			memset((char *)descs, 0, dlen);
			flush_cache_pre_dma_out(descs, dlen);
			tp->rx_ring = (struct tulip_rx_desc *)CACHED_TO_UNCACHED(descs);
			descs += (sizeof(struct tulip_rx_desc) * RX_RING_SIZE);
			tp->tx_ring = (struct tulip_tx_desc *)CACHED_TO_UNCACHED(descs);
	}
#endif
	dev->priv = tp;
	...
	case DC21140:  case DC21142:
		...
		outl(0x82420000, ioaddr + CSR6);
		outl(0x0, ioaddr + CSR13);
		outl(0x1, ioaddr + CSR13);
		outl(0x0001, ioaddr + CSR12);
		break;
	case DC21040:
		outl(0x00000000, ioaddr + CSR13);
...
static struct fixups {
  ...
  {"Cobalt 27", 0, 0x40, 0x10, {0x1e00, 0x0000,0x009,0x011f,
								0x0380, 0x0000,
								0x0000, 0x2800, }},
  {"Cobalt 27", 0, 0x10, 0xE0, {0x1e00, 0x0000,0x009,0x011f,
                                                                0x0380, 0x0000,
                                                                0x0000, 0x2800,
}},
...
static void parse_eeprom(struct device *dev, int chip_id)
{
	...
	int i;
	/* COBALT LOCAL: This driver is not Alpha quality, due to all
	 *               the internal testing and fixes Cobalt has done
	 *               to it, so don't print this misleading message. -DaveM
	 */
#if 0
	{
	...
	for (i = 0; i < 8; i ++)
	  if (ee_data[i] != ee_data[16+i])
		break                                                                                                                     ;
#if defined(__mips__)
	if ( ioaddr != 0x10108000 || i >= 8 ) {			// Our EEPROM is REALLY broken
#else
	if (i >= 8) {
#endif
	  if (ee_data[0] == 0xff) {
...
#ifdef CONFIG_COBALT_27
							printk(" %4.4x", mdio_read(ioaddr, 1, mii_reg));
#else
...
#if defined(__mips__)
	// What should this be?
	outl(0x00200000 | 0x4000, ioaddr + CSR0);
#else
...
#if defined(__mips__)
	if (request_irq(dev->irq, &tulip_interrupt,SA_INTERRUPT|SA_SHIRQ,
...
static void tulip_timer(unsigned long data)
{
	...
	case DC21140:  case DC21142: {
	  ...
	  case 3:					/* 21142 MII */
	  {
			  ...
			  mii_reg5 = mdio_read(ioaddr, 1, 5);
			  if (((mii_reg5 & 0x0100) != 0x0000 ||
				   (mii_reg5 & 0x00c0) == 0x0040)) {
					  if (tp->full_duplex == 0 || (tp->csr6 & 0x0200) == 0) {
						   tp->full_duplex = 1;
						   tp->csr6 |= 0x0200;
						   outl(tp->csr6 | 0x0002, ioaddr + CSR6);
						   outl(tp->csr6 | 0x2002, ioaddr + CSR6);
...
tulip_linkcheck(void *cookie)
{
		...
		if (cobalt_is_raq != 0 ||
			...
			0
				...
				unsigned short link_status = mdio_read(dev->base_addr, 1, 1);
				if ((link_status & (1 << 2)) != 0)
						return 1;
				return 0;
		} else {
				int ioaddr = dev->base_addr;
		...
		if (csr5 & 0x0007) {		/* Tx-done interrupt */
			...
			for (dirty_tx = lp->dirty_tx; dirty_tx < lp->cur_tx; dirty_tx++) {
				...
				if (status & 0x8000) {
					...
					if (tulip_debug > 1) {
						printk("%s: Transmit error, Tx status %8.8x CSR5[%8.8x] CSR6[%8.8x] "
...
/* The following scheme is radically different from the standard Tulip
 * drivers, which are full of several races:
 *
 * 1) We never get rid of the packets which serve as the buffers the
 *    chip uses to receive.
 * 2) We always allocate a new buffer to send off to the networking.
 *    If the allocation fails, we silently drop the packet.
 * 3) In all cases we finish handling of each entry by giving it
 *    back to the chip (by setting the ownership bit).
 */
...
tulip_rx(struct device *dev)
{
	...
	while ((status = lp->rx_ring[entry].status) >= 0) {
		if ((status & 0x0300) != 0x0300) {
			...
			if(skb == NULL) {
					...
					skb->dev = dev;
					skb_reserve(skb, 2);
					eth_copy_and_sum(skb, bus_to_virt(lp->rx_ring[entry].buffer1), pkt_len, 0);
#ifdef CONFIG_COBALT_27
					/* COBALT LOCAL: Kick out any matching lines in the cache so
					 *               that, next time, we see what the card wrote to
					 *               memory not what we just read into the cache.  -DaveM
					 */
					flush_cache_post_dma_in(
							(unsigned long)bus_to_virt(lp->rx_ring[entry].buffer1),
							pkt_len);
#endif
					skb_put(skb, pkt_len);
		...
		/* Pass ownership back to the chip. */
