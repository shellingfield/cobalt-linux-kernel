...
/*
	...
	Modified at Cobalt by David Miller for MIPS cache model, etc.
	0.91 Changes merged by Chris Johnson (cjohnson@cobaltnet.com)
*/
...
#define REVERSE_PROBE_ORDER	/* Cobalt - force onboard before PCI slot */
...
#define TX_TIMEOUT  (2*HZ)	/* Cobalt wants to recover faster */
	/* 
	 * We use 1600 instead of the MTU because some
	 * collisions result in concatenated packets
	 */
#define PKT_BUF_SZ	1600

	/*
	 * Cobalt systems seem to need a little extra space.
	 * Completely unclear why - it's as though the tulip
	 * hardware wants to stomp on memory for fun.
	 */
#ifdef __mips__
#define PKT_BUF_FUDGE	4
#else
#define PKT_BUF_FUDGE	0
#endif
...
#include <asm/pgtable.h>		/* for flush_cache_pre_dma_out */
...
static int tulip_linkcheck(void *cookie);
#endif
...
#ifdef BOOTLOADER
struct device * first_tulip_dev = NULL;
struct device * second_tulip_dev = NULL;
#endif
...
int tulip_probe(struct device *dev)
{
	...
	for (;pci_index < 0xff; pci_index++) {
		...
		if (dev) {
			...
			if (!first_tulip_dev)
				first_tulip_dev = dev;
			else if (!second_tulip_dev)
				second_tulip_dev = dev;
#endif
		}
		dev = 0;
		cards_found++;
								   ...
								   int chip_idx, int board_idx)
{
	...
	dev = init_etherdev(dev, sizeof(*tp));
...
#ifdef __mips__
		if (ioaddr == 0x10108000)	/* PCI board does it right */
#endif
		    for (i = 0; i < 8; i ++)
	...
	tp = (void *)(((long)kmalloc(sizeof(*tp) + 7, GFP_KERNEL | GFP_DMA) + 7) & ~7);
...
#endif
#ifdef CONFIG_COBALT_27
        lcd_register_linkcheck_func(dev->name[3] - '0', tulip_linkcheck, dev);
#endif
...
/*
 * Here is the deal:
 * Some cards have broken roms, so we use the following table to kludge
 * up the correct values.  The "addr" bytes are expected to be the first
 * few bytes in the ROMs (vendorID and subsystem ID).  If we find them,
 * we use the "newtable" to replace the 32 shorts at byte offset 26 in
 * the ROM.  This means we overwrite the controller specific information,
 * which has the following layout:

    struct controler_info {
	char dev_num;
	short leaf_offset;
    } ctrl[NUM_CONTROLLERS]
    char rest[32 - sizeof(ctlr)];
 
 * NUM_CONTROLLERS is board specific, and the data structure is packed
 * so the 'short leaf_offset' alternate between being good and bad
 * alignment.  That's why there is no 'C' structure.
 *
 * The leaf_offset is added to the eeprom base to find the media "leaf"
 * information for the controller.  This is an array of PHY specific 
 * structures that detail interesting features about the hardware.
 * The leaf structure is similarly demented, and starts with a length
 * field, and a type.
 * 
 * And just to confuse you more, the flavors of leaves differ between
 * the various flavors of tulips.
 *
 * And finally, www.digital.com is useless - Compaq sold the chip stuff
 * to Intel.  See:
 *	http://developer.intel.com/design/network/drivers/srom_409.pdf
 * Or go near there and rummage around.
 *
 * cj*
 */
...
static void parse_eeprom(struct device *dev)
{
	...
	/*
	 * Cobalt:
	 *    Our EEPROMs are specification challenged, so we check
	 *    to see if the current device is not in the PCI slot.
	 *    PCI cards fend for themselves, otherwise we kludge.
	 */
	if(ioaddr != 0x10108000) {	/* not PCI slot */
		ee_data[19] = 1;	/* set One controller... TJS */
		i = 8;			/* make us fall into the right code */
	}
	else
#endif
	...
	if (ee_data[27] == 0) {		/* No valid media table. */
		...
		for (i = 0; i < count; i++) {
			...
			if (tulip_debug > 1  &&  leaf->media == 11) {
				...
				unsigned char init_len, reset_len;
				unsigned short capabilities;

				init_len = bp[1];
				reset_len = bp[2 + init_len * 2];
				capabilities = get_unaligned((u16*)
					&bp[3 + 2 * init_len + 2 * reset_len]);

				printk(KERN_INFO "%s:  MII interface PHY %d, setup/reset "
					   "sequences %d/%d long, capabilities %4.4x\n",
					   dev->name, bp[0],  /* PHY # */
					   init_len, reset_len, capabilities);
			}
...
tulip_open(struct device *dev)
{
	...
	if (tulip_tbl[tp->chip_id].flags & MC_HASH_ONLY) {
		...
		*setup_frm++ = eaddrs[2]; *setup_frm++ = eaddrs[2];
		...
		flush_cache_pre_dma_out((unsigned long)tp->setup_frame,
	...
	if (mtable) {
		...
		case 1: case 3: {
			...
			if (mleaf->type == 3) {	/* 21142 */
				...
				reset_length = * (char *)
					&init_sequence[init_length];

				reset_sequence = (u16 *)
				    ( 1 + (char *) &init_sequence[init_length]);
...
tulip_linkcheck(void *cookie)
{
    ...
    /*
     * If we have a MII, check the PHY
     */
    if (tp->mtable && tp->mtable->has_mii == 1) {

	link_status = mdio_read(dev, 1, 1);

	return (link_status & (1 << 2)) != 0;
    }

    /*
     * Check the chip 10 and 100 Mbit/s link
     * (not obvious how 100 Mbit can blink if there is no MII)
     */
    link_status = inl(ioaddr + CSR12);

    return (link_status & 6) != 0;
}
...
static void tulip_init_ring(struct device *dev)
{
...
#if defined(__mips__)
	/*
	 * For simplicity, we want out descriptors to be in
	 * non-cached space since otherwise we need to flush
	 * after every update.  Non-coherent DMA sucks.
	 */
	if ( ! tp->tx_ring || ! tp->rx_ring) {
		int dlen = 0;
		unsigned long descs;

		if ( ! tp->tx_ring)
			dlen += sizeof(struct tulip_tx_desc) * TX_RING_SIZE;

		if ( ! tp->rx_ring)
			dlen += sizeof(struct tulip_rx_desc) * RX_RING_SIZE;

		/*
		 * Fudge up by 32 bytes to keep
		 * universe alligned nicely.
		 */
		descs = (unsigned long)
			kmalloc(dlen + 32, GFP_KERNEL | GFP_DMA);

		descs = (descs + (32UL - 1UL)) & ~(32UL - 1UL);
		memset((char *)descs, 0, dlen);

		flush_cache_pre_dma_out(descs, dlen);

		if ( ! tp->tx_ring) {
			tp->tx_ring = (struct tulip_tx_desc *)
				CACHED_TO_UNCACHED(descs);
			descs += sizeof(struct tulip_tx_desc) * TX_RING_SIZE;
		}

		if ( ! tp->rx_ring) {
			tp->rx_ring = (struct tulip_rx_desc *)
				CACHED_TO_UNCACHED (descs);        
		}       
	}
#endif          
	...
	for (i = 0; i < RX_RING_SIZE; i++) {
		...
		struct sk_buff *skb = dev_alloc_skb(PKT_BUF_SZ + PKT_BUF_FUDGE);
		tp->rx_skbuff[i] = skb;
		if (skb == NULL)
			break;
		if (skb->list)
		    printk("init_ring got sk_buff already on list\n");

		skb->dev = dev;			/* Mark as being used by this device. */

#ifdef CONFIG_COBALT_27
		/* COBALT LOCAL: Kick out any matching lines in the cache. -DaveM */
		flush_cache_post_dma_in((unsigned long)skb->tail, PKT_BUF_SZ);
#endif
...
tulip_start_xmit(struct sk_buff *skb, struct device *dev)
{
	...
	save_flags(cpu_flags);		/* XXX deleted in 0.91 ??? */
	cli();
	...
	flush_cache_pre_dma_out((unsigned long)skb->data, skb->len);
...
static void tulip_interrupt(int irq, void *dev_instance, struct pt_regs *regs)
{
	...
	if (dev->interrupt) {
		...
		/* What to do?  If we return, we will just get hit with
		 * another interupt.  We ought to panic
		 */
		return;
	}
	...
	do {
		...
		if (csr5 & (TxNoBuf | TxDied | TxIntr)) {
			...
			for (dirty_tx = tp->dirty_tx; tp->cur_tx - dirty_tx > 0;
				...
				if (status & 0x8000) {
					...
					if (tulip_debug > 1) {
						printk(KERN_DEBUG "%s: Transmit error, Tx status %8.8x CSR5[%8.8x] CSR6[%8.8x] "
							   "CSR12[%8.8x] CSR14[%8.8x]\n",
							   dev->name, status, csr5, inl(ioaddr + CSR6),
							   inl(ioaddr + CSR12), inl(ioaddr + CSR14));
					}
#endif
					tp->stats.tx_errors++;
		...
		if (--work_budget < 0) {
			/* This seems wrong since we will just get
			 * aother interrupt, but in fact this allows
			 * the network "thread" to run.
			 * cj*
			 */
			if (tulip_debug > 1)
...
#ifdef CONFIG_COBALT_27
				/*
				 * Scrub the cache to avoid stale data in
				 * the next packet delivered in this slot.
				 * (This should be #ifdef NON_COHERENT_DMA)
				 */
				flush_cache_post_dma_in((unsigned long)
	...
	for (; tp->cur_rx - tp->dirty_rx > 0; tp->dirty_rx++) {
		...
		if (tp->rx_skbuff[entry] == NULL) {
			...
			skb = tp->rx_skbuff[entry] =
				    dev_alloc_skb(PKT_BUF_SZ + PKT_BUF_FUDGE);
			if (skb == NULL)
				break;
			skb->dev = dev;			/* Mark as being used by this device. */
...
tulip_close(struct device *dev)
{
	...
	/*
	 * Nuke timer first - it gets confused if it runs
