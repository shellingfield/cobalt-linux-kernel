...
static char *version =
"3c59x.c:v0.99C 4/26/98 Donald Becker http://cesdis.gsfc.nasa.gov/linux/drivers/vortex.html\n";

static char *cobalt_version =
"cobalt3com.c:v1.00 4/26/98 David S. Miller (davem@dm.cobaltmicro.com)\n";
...
#endif
#ifdef CONFIG_COBALT_27
static int vortex_linkcheck(void *cookie);
#endif
...
#ifdef CONFIG_COBALT_27
				/* COBALT LOCAL: Set it a bit higher, I get transmit timeouts
				 *               otherwise.  -DaveM
				 * 248 hangs certain newer 3Com cards, lower
				 * it to 64. It now works with all cards we
				 * have. -- mingo
				 */
				u8 new_latency = (device&0xff00) == 0x5900 ? 64 : 64;
#else
					...
					int device_id, int option, int card_idx)
{
	...
	flush_cache_pre_dma_out((unsigned long)dev->priv, sizeof(struct vortex_private));
	dev->priv = (void *)CACHED_TO_UNCACHED(dev->priv);
#endif
	...
	vortex_probe1(dev);
#ifdef CONFIG_COBALT_27
	lcd_register_linkcheck_func(dev->name[3] - '0', vortex_linkcheck, dev);
#endif
#endif /* MODULE */
	return dev;
}
...
#ifdef SA_SHIRQ
#ifdef CONFIG_COBALT_27
	/* COBALT LOCAL: We don't ever have shared interrupts nor do we
	 * handle them.   -DaveM
	 */
	if (request_irq(dev->irq, &vortex_interrupt, SA_INTERRUPT,
					vp->product_name, dev)) {
		return -EAGAIN;
	}
#else
...
#ifdef CONFIG_COBALT_27
static int
vortex_linkcheck(void *cookie)
{
	struct device *dev = (struct device *)cookie;
	struct vortex_private *vp = (struct vortex_private *) dev->priv;
	int mstatus, old_window, retval, ioaddr = dev->base_addr;
	unsigned long flags;
	...
	EL3WINDOW(4);
	mstatus = inw(ioaddr + Wn4_Media);
	switch (dev->if_port) {
	case XCVR_10baseT:
	case XCVR_100baseTx:
	case XCVR_100baseFx:
		if (mstatus & Media_LnkBeat)
			retval = 1;
		break;
	case XCVR_MII:
...
static void vortex_interrupt IRQ(int irq, void *dev_id, struct pt_regs *regs)
{
	...
	do {
		...
		if (status & DownComplete) {
			...
			while (lp->cur_tx - dirty_tx > 0) {
				...
				if (lp->tx_skbuff[entry]) {
					DEV_FREE_SKB(lp->tx_skbuff[entry]);
		...
		if (status & DMADone) {
			...
			clear_bit(0, (void*)&dev->tbusy);
			DEV_FREE_SKB(lp->tx_skb); /* Release the transfered buffer */
			mark_bh(NET_BH);
		}
#endif
...
boomerang_rx(struct device *dev)
{
	...
	while ((rx_status = vp->rx_ring[entry].status) & RxDComplete) {
		if (rx_status & RxDError) { /* Error, update stats. */
			 ...
			 */
			flush_cache_post_dma_in((unsigned long)bus_to_virt(vp->rx_ring[entry].addr),
									pkt_len);
#endif
