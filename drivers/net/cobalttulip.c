...
#define SMP_CHECK
static const char version[] = "tulip.c:v0.89B 4/19/98 becker@cesdis.gsfc.nasa.gov\n";

static const char cobalt_version[] = "cobalttulip.c:v1.00 4/26/98 davem@dm.cobaltmicro.com\n";
...
/* COBALT LOCAL: Needed because our mips is not DMA coherent.  Also
 *               we need to find the on-board Tulip before any such
 *               adapters in the PCI expansion slot of the Qube.  -DaveM
 */
#define CACHED_TO_UNCACHED(x)         (((unsigned long)(x) & (unsigned long)0x1fffffff) + KSEG1)
#define REVERSE_PROBE_ORDER
#endif
...
#if defined(__alpha__) || defined(CONFIG_COBALT_27)
...
#ifdef CONFIG_COBALT_27
	/* COBALT LOCAL: We wish to have the upper layer go through another
	 *               round of probing to find possible other card types
	 *               in the PCI expansion slot of the Qube, thus this.  -DaveM
	 */
	if(called == 0) {
			called = 1;
			return 0;
								   ...
								   int chip_id, int board_idx)
{
...
#ifdef CONFIG_COBALT_27
		/* COBALT LOCAL: Yes, our on-board Tulip is weird... -DaveM */
		if (ioaddr != 0x10108000) {
			for (i = 0; i < 6; i ++) {
	...
	flush_cache_pre_dma_out((unsigned long)tp, sizeof(*tp));
	tp = (void *)CACHED_TO_UNCACHED(tp);
#endif
	dev->priv = tp;
...
#ifdef CONFIG_COBALT_27
  /* COBALT LOCAL: These are for the RAQ. -DaveM */
  {"Cobalt RAQ", 0, 0x40, 0x10, {0x1e00, 0x0000,0x009,0x011f,
								 0x0380, 0x0000,
								 0x0000, 0x2800, }},
  {"Cobalt RAQ", 0, 0x10, 0xE0, {0x1e00, 0x0000,0x009,0x011f,
								 0x0380, 0x0000,
								 0x0000, 0x2800}},
#endif
  {0, 0, 0, 0, {}}};
...
static void parse_eeprom(struct device *dev)
{
...
#ifdef CONFIG_COBALT_27
	/* COBALT LOCAL: Broken EEPROM, force 1 controller. */
	if(ioaddr != 0x10108000)
		ee_data[19] = 1;
#endif
	...
	for (i = 0; i < 8; i ++)
		if (ee_data[i] != ee_data[16+i])
			break;
#ifdef CONFIG_COBALT_27
	/* COBALT LOCAL: The EEPROM on the Qube is really broken. */
	if (ioaddr != 0x10108000 || i >= 8)
#else
	if (i >= 8)
...
#ifdef CONFIG_COBALT_27
	  /* COBALT LOCAL: We only need fixups on Alpha 27 boards
	   *               with 21143's in them.  -DaveM
	   */
...
subsequent_board:
	...
	if (ee_data[27] == 0) {		/* No valid media table. */
		...
		if (tp->chip_id == DC21140  ||  tp->chip_id == MX98713)
			csr12dir = *p++;
#ifdef CONFIG_COBALT_27
		/* Hidden treasure done right... -DaveM */
		if (doing_fixup == 1)
			p++;
#endif
		count = *p++;
...
static int mdio_read(struct device *dev, int phy_id, int location)
{
	...
	int retval = 0;
	int mdio_addr = dev->base_addr + CSR9;
...
tulip_open(struct device *dev)
{
	...
	outl(0x01A00000 | 0x8000, ioaddr + CSR0);
...
media_picked:
	...
	/* COBALT LOCAL: This causes severe problems with the 21143 chips
	 *               in our RaQ boards.  The result of not doing this is
	 *				 to get the exact behavior all older Tulip drivers
	 *				 had in these case. (ie. select_media() always called)
	 */
...
#ifdef CONFIG_COBALT_27
		/* COBALT LOCAL: This is what all older Tulip drivers did, if
		 *               we use the new code below, RaQ board chips fail.
		 */
		case 3: {
			int init_length = p[1];
...
tulip_linkcheck(void *cookie)
{
		...
		struct tulip_private *tp = (struct tulip_private *) dev->priv;
		int ioaddr = dev->base_addr;
		unsigned int link_status = inl(ioaddr + CSR12);

		/* This driver only used on 2700 hardware.  Although we should
		 * do something more robust for expansion slot Tulip PCI cards.  --DaveM
		 */
		if ((link_status & ((1<<1) | (1<<2))) != 0)
			return 1;
		return 0;
}
#endif /* CONFIG_COBALT_27 */
...
#ifdef CONFIG_COBALT_27
/* COBALT LOCAL: See caller of this below. 8-( --DaveM */
static void tulip_complete_reset(struct device *dev)
{
		struct tulip_private *tp = (struct tulip_private *)dev->priv;
		unsigned long flags;
		int ioaddr = dev->base_addr;
		...
		/* Step1: Stop Tx/Rx engines and clear missed-packet counter. */
		outl(inl(ioaddr + CSR6) & ~0x2002, ioaddr + CSR6);
		(volatile)inl(ioaddr + CSR8);

		/* Step2: Reset the xcvr interface and turn on heartbeat. */
		outl(0x00000000, ioaddr + CSR13);
		outl(0xFFFFFFFF, ioaddr + CSR14);
		...
		/* Step3: Reset the chip. */
		outl(0x00000001, ioaddr + CSR0);
		udelay(2);

		/* Step4: De-assert reset and set cache alignment. */
		outl(0x00200000 | 0xE000, ioaddr + CSR0);

		/* Step5: Zap the tx/rx rings, free'ing up pending SKB's in the tx ring. */
		for(i = 0; i < RX_RING_SIZE; i++) {
				struct sk_buff *skb;

				tp->rx_ring[i].status = 0x80000000;
		...
		for(i = 0; i < TX_RING_SIZE; i++) {
				...
				/* Fix up tx-ring forward ptr just in case... */
		...
		/* Step6: Setup filter address table in tx ring */
		{
				u16 *eaddrs = (u16 *)dev->dev_addr;
		...
		/* Step7: Set rx/tx descriptor ring pointers. */
		outl(virt_to_bus(tp->rx_ring), ioaddr + CSR3);
		outl(virt_to_bus(tp->tx_ring), ioaddr + CSR4);
		...
		/* Step9: Start tx engine so it eats the setup frame. */
		outl(tp->csr6, ioaddr + CSR6);
		outl(tp->csr6 | 0x2000, ioaddr + CSR6);

		/* Step10: Clear SW interrupt synchronization state. */
		dev->tbusy = 0;
		dev->interrupt = 0;
		dev->start = 1;

		/* Step11: Enable interrupts and rx engine. */
		outl(0x0001fbff, ioaddr + CSR7);
		outl(tp->csr6 | 0x2002, ioaddr + CSR6);
		outl(0, ioaddr + CSR2);		/* Rx poll demand */

		/* Step12: Get linkbeat/media-detect timer going. */
		del_timer(&tp->timer);
		tp->timer.expires = RUN_AT((24*HZ)/10);
		tp->timer.data = (unsigned long) dev;
		tp->timer.function = &tulip_timer;
		add_timer(&tp->timer);
...
static void tulip_tx_timeout(struct device *dev)
{
  ...
  if (tp->mtable && tp->mtable->has_mii) {
	...
	tp->mediasense = 1;
#ifdef CONFIG_COBALT_27
	/* COBALT LOCAL: The transmitter of the DC21041 can lock up totally,
	 *               if it is shut out from the PCI bus for too many
	 *               cycles.  I've witnessed this live with our PCI bus
	 *               logic analyzer, in our case it was the VIA IDE disk
	 *               controller streaming DMA causing the starvation.  -DaveM
	 */
	tulip_complete_reset(dev);
#else
...
#ifdef CONFIG_COBALT_27
					/* COBALT LOCAL: Some collision hacks. -DaveM */
					if (status & 0x0100) {
#ifdef ETHER_STATS
							tp->stats.collisions16++;
#endif
							tp->stats.collisions += 16;
					}
					if (status & 0x0200)
							tp->stats.collisions++;
#else
...
#endif
#ifdef CONFIG_COBALT_27
					/* COBALT LOCAL: Even more collision hacks. -DaveM */
					if (status & 0x78)
							tp->stats.collisions++;
#else
...
#warning Code untested
...
#if !defined(final_version) && !defined(CONFIG_COBALT_27)
				{
			 ...
			 *               we see what the card wrote to memory.  -DaveM
			 */
			flush_cache_post_dma_in(
				(unsigned long)bus_to_virt(tp->rx_ring[entry].buffer1),
				pkt_len);
#endif
			netif_rx(skb);
			dev->last_rx = jiffies;
