...
isdn_net_unreachable(struct device *dev, struct sk_buff *skb, char *reason)
{
	...
	if(skb != NULL) {
		...
		isdn_putlog(KERN_DEBUG "isdn_net: %s: %s, send ICMP %s\n",
			   dev->name,
		...
		isdn_putlog(KERN_DEBUG "isdn_net: %s: %s\n",
			   dev->name,
			   (reason != NULL) ? reason : "reason unknown");
	}
...
isdn_net_autohup()
{
	...
	while (p) {
		...
		if (dev->net_verbose > 3)
			isdn_putlog(KERN_DEBUG "%s: %d bogocps\n", l->name, l->cps);
				...
				if (l->hupflags & ISDN_MANCHARGE &&
					...
					if (l->hupflags & ISDN_CHARGEHUP) {
						if (l->hupflags & ISDN_WAITCHARGE) {
							isdn_putlog(KERN_DEBUG "isdn_net: Hupflags of %s are %X\n",
							       l->name, l->hupflags);
							isdn_net_hangup(&p->dev);
	...
	if (p) {
		...
		switch (cmd) {
			...
			case ISDN_STAT_DHUP:
				...
				if (lp->flags & ISDN_NET_CONNECTED) {
					isdn_putlog(KERN_INFO "%s: remote %s (%d)\n", lp->name,
						lp->dialstate?"abort":"hangup", lp->dialstate);
					isdn_putlog(KERN_INFO "%s: Chargesum is %d\n", lp->name,
						lp->charge);
isdn_putlog(KERN_DEBUG "idx=%d drv=%d ch=%d\n",idx, lp->isdn_device, lp->isdn_channel);
					if (!lp->dialstate) {
						...
						switch (lp->dialstate) {
							...
							case 12:
								{
									isdn_ctrl cmd;
									isdn_putlog(KERN_INFO "%s: got reject, waiting for callback ...\n", p->local.name);
			...
			case ISDN_STAT_BCONN:
				...
				switch (lp->dialstate) {
					...
					case 12:
						...
						isdn_putlog(KERN_INFO "isdn_net: %s connected\n", lp->name);
						/* If first Chargeinfo comes before B-Channel connect,
						 * we correct the timestamp here.
						 */
						lp->chargetime = jiffies;
						isdn_putlog(KERN_DEBUG "isdn_net: chargetime of %s now %d\n",
			...
			case ISDN_STAT_CINF:
				...
				lp->chargetime = jiffies;
				isdn_putlog(KERN_DEBUG "isdn_net: Got CINF chargetime of %s now %d\n",
				       lp->name, lp->chargetime);
				return 1;
		}
	}
...
isdn_net_dial(void)
{
	...
	while (p) {
		...
		if (p->local.dialstate)
			isdn_putlog(KERN_DEBUG "%s: dialstate=%d\n", p->local.name, p->local.dialstate);
#endif
		...
		switch (p->local.dialstate) {
			...
			case 1:
				...
				restore_flags(flags);
				if (!p->local.dial) {
					isdn_putlog(KERN_WARNING "%s: phone number deleted?\n",
					       p->local.name);
					isdn_net_hangup(&p->dev);
					break;
				}
			...
			case 3:
				...
				if (!strcmp(p->local.dial->num, "LEASED")) {
					restore_flags(flags);
					p->local.dialstate = 4;
					isdn_putlog(KERN_INFO "%s: Open leased line ...\n", p->local.name);
				} else {
					if(p->local.dialtimeout > 0)
					...
					isdn_putlog(KERN_INFO "%s: dialing %d %s...\n", p->local.name,
					       p->local.dialretry, cmd.parm.setup.phone);
					p->local.dtimer = 0;
#ifdef ISDN_DEBUG_NET_DIAL
					isdn_putlog(KERN_DEBUG "dial: d=%d c=%d\n", p->local.isdn_device,
			...
			case 6:
				 ...
				 */
#ifdef ISDN_DEBUG_NET_DIAL
				isdn_putlog(KERN_DEBUG "dialtimer2: %d\n", p->local.dtimer);
#endif
				if (p->local.dtimer++ > ISDN_TIMER_DTIMEOUT10)
					p->local.dialstate = 3;
				anymore = 1;
			...
			case 7:
				 ...
				 */
#ifdef ISDN_DEBUG_NET_DIAL
				isdn_putlog(KERN_DEBUG "dialtimer4: %d\n", p->local.dtimer);
#endif
				cmd.driver = p->local.isdn_device;
				cmd.command = ISDN_CMD_SETL2;
				cmd.arg = p->local.isdn_channel + (p->local.l2_proto << 8);
			...
			case 12:
				...
				if (p->local.dtimer++ > p->local.cbdelay) 
				{
					isdn_putlog(KERN_INFO "%s: hangup waiting for callback ...\n", p->local.name);
					p->local.dtimer = 0;
					p->local.dialstate = 13;
					cmd.driver = p->local.isdn_device;
					cmd.command = ISDN_CMD_HANGUP;
				...
				break;
			default:
				isdn_putlog(KERN_WARNING "isdn_net: Illegal dialstate %d for device %s\n",
...
isdn_net_log_packet(u_char * buf, isdn_net_local * lp)
{
	...
	switch (proto) {
		case ETH_P_IP:
			...
			isdn_putlog(KERN_INFO "OPEN: %d.%d.%d.%d -> %d.%d.%d.%d%s\n",
			       p[12], p[13], p[14], p[15],
			       p[16], p[17], p[18], p[19],
			       addinfo);
			break;
		case ETH_P_ARP:
			isdn_putlog(KERN_INFO "OPEN: ARP %d.%d.%d.%d -> *.*.*.* ?%d.%d.%d.%d\n",
			       p[14], p[15], p[16], p[17],
			       p[24], p[25], p[26], p[27]);
			break;
	}
...
/*
isdn_putlog("reject: jiffies=%ld, started=%ld, timeout=%d, wait=%ld, timer=%ld\n", jiffies, lp->dialstarted, lp->dialtimeout, lp->dialwait, lp->dialwait_timer);
*/
						/*
						isdn_putlog(KERN_WARNING "isdn_net: Dial rejected %s, packet dropped\n",
							ndev->name);
						*/
				 ...
				 */
				if (lp->first_skb) {
					isdn_putlog(KERN_DEBUG "isdn_net_start_xmit: First skb already set!\n");
					dev_kfree_skb(lp->first_skb, FREE_WRITE);
					lp->first_skb = NULL;
				}
				lp->first_skb = skb;
...
#endif
		default:
			isdn_putlog(KERN_WARNING "%s: unknown encapsulation, dropping\n",
			       lp->name);
...
#endif
		case ISDN_NET_ENCAP_RAWIP:
			isdn_putlog(KERN_WARNING "isdn_net_header called with RAW_IP!\n");
			len = 0;
			break;
		case ISDN_NET_ENCAP_IPTYP:
			/* ethernet type field */
...
isdn_net_init(struct device *ndev)
{
	...
	if (ndev == NULL) {
		isdn_putlog(KERN_WARNING "isdn_net_init: dev = NULL!\n");
		return -ENODEV;
	}
	if (ndev->priv == NULL) {
		isdn_putlog(KERN_WARNING "isdn_net_init: dev->priv = NULL!\n");
		return -ENODEV;
	}
	ether_setup(ndev);
#if (LINUX_VERSION_CODE < 0x02010F)
...
isdn_net_swapbind(int drvidx)
{
	...
	isdn_putlog(KERN_DEBUG "n_fi: swapping ch of %d\n", drvidx);
#endif
	p = dev->netdev;
	while (p) {
...
#ifdef ISDN_DEBUG_NET_ICALL
	isdn_putlog(KERN_DEBUG "n_fi: usage of %d and %d\n", i1, i2);
#endif
...
isdn_net_find_icall(int di, isdn_ctrl *c, int idx)
{
	...
	if (!setup->phone[0]) {
		nr[0] = '0';
		nr[1] = '\0';
		isdn_putlog(KERN_INFO "isdn_net: Incoming call without OAD, assuming '0'\n");
	} else
		strcpy(nr, setup->phone);
	si1 = (int) setup->si1;
	si2 = (int) setup->si2;
	if (!setup->eazmsn[0]) {
		isdn_putlog(KERN_WARNING "isdn_net: Incoming call without CPN, assuming '0'\n");
		eaz = "0";
	} else
		eaz = setup->eazmsn;
	if (dev->net_verbose > 1)
		isdn_putlog(KERN_INFO "isdn_net: call from %s,%d,%d -> %s\n", nr, si1, si2, eaz);
	/* Accept only calls with Si1 = 7 (Data-Transmission) */
	if (si1 != 7) {
		if (dev->net_verbose > 1)
			isdn_putlog(KERN_INFO "isdn_net: Service-Indicator not 7, ignored\n");
		restore_flags(flags);
		return 0;
	}
	n = (isdn_net_phone *) 0;
	ematch = 0;
#ifdef ISDN_DEBUG_NET_ICALL
	isdn_putlog(KERN_DEBUG "n_fi: di=%d ch=%d idx=%d usg=%d\n", di, ch, idx,
	       dev->usage[idx]);
#endif
	swapped = 0;
	...
	for (p = dev->netdev; p; p = (isdn_net_dev *) p->next) {
...
#ifdef ISDN_DEBUG_NET_ICALL
		isdn_putlog(KERN_DEBUG "n_fi: if='%s', l.msn=%s, l.flags=%d, l.dstate=%d\n",
		...
		if ( (!(p->local.flags & ISDN_NET_CONNECTED) &&	/* not connected  */
...
#ifdef ISDN_DEBUG_NET_ICALL
			isdn_putlog(KERN_DEBUG "n_fi: match1, pdev=%d pch=%d\n",
...
#ifdef ISDN_DEBUG_NET_ICALL
						isdn_putlog(KERN_DEBUG "n_fi: ch is 0\n");
#endif
...
#ifdef ISDN_DEBUG_NET_ICALL
								isdn_putlog(KERN_DEBUG "n_fi: 2nd channel is down and bound\n");
#endif
...
#ifdef ISDN_DEBUG_NET_ICALL
								isdn_putlog(KERN_DEBUG "n_fi: 2nd channel is down and unbound\n");
#endif
...
#ifdef ISDN_DEBUG_NET_ICALL
							isdn_putlog(KERN_DEBUG "n_fi: final check\n");
#endif
							if ((dev->usage[idx] & ISDN_USAGE_EXCLUSIVE) &&
							    ((p->local.pre_channel != ch) ||
							     (p->local.pre_device != di))) {
#ifdef ISDN_DEBUG_NET_ICALL
								isdn_putlog(KERN_DEBUG "n_fi: final check failed\n");
#endif
								continue;
							}
						}
...
#ifdef ISDN_DEBUG_NET_ICALL
						isdn_putlog(KERN_DEBUG "n_fi: already on 2nd channel\n");
#endif
						continue;
					}
				}
...
#ifdef ISDN_DEBUG_NET_ICALL
			isdn_putlog(KERN_DEBUG "n_fi: match2\n");
#endif
			n = p->local.phone[0];
			...
			if (n || (!(p->local.flags & ISDN_NET_SECURE))) {
				isdn_net_local *lp = &(p->local);
#ifdef ISDN_DEBUG_NET_ICALL
				isdn_putlog(KERN_DEBUG "n_fi: match3\n");
#endif
				...
				if (ISDN_NET_DIALMODE(*lp) == ISDN_NET_DM_OFF) {
					restore_flags(flags);
					isdn_putlog(KERN_INFO "incoming call, interface `%s' dialmode `off' -> rejected\n",
					       lp->name);
					return 3;
				}
				/*
				...
				if (!p->dev.start) {
					restore_flags(flags);
					isdn_putlog(KERN_INFO "incoming call, interface `%s' down -> rejected\n",
					       lp->name);
					return 3;
				}
				...
				if (lp->master) {
					...
					isdn_putlog(KERN_DEBUG "ICALLslv: %s\n", lp->name);
					isdn_putlog(KERN_DEBUG "master=%s\n", mlp->name);
					if ((mlp->flags & ISDN_NET_CONNECTED) && (!mlp->dialstate)) {
						isdn_putlog(KERN_DEBUG "master online\n");
						...
						isdn_putlog(KERN_DEBUG "master offline\n");
					/* Found parent, if it's offline iterate next device */
					isdn_putlog(KERN_DEBUG "mlpf: %d\n", mlp->flags & ISDN_NET_CONNECTED);
					if (!(mlp->flags & ISDN_NET_CONNECTED) || mlp->dialstate) {
						continue;
					}
				}
				if (lp->flags & ISDN_NET_CALLBACK) {
					...
					if (ISDN_NET_DIALMODE(*lp) != ISDN_NET_DM_AUTO) {
						restore_flags(flags);
						isdn_putlog(KERN_INFO "incoming call for callback, interface `%s' dialmode not `auto' -> rejected\n",
						       lp->name);
						return 3;
					}
					isdn_putlog(KERN_DEBUG "%s: call from %s -> %s, start callback\n",
					       lp->name, nr, eaz);
					if (lp->phone[1]) {
						/* Grab a free ISDN-Channel */
						if ((chi = isdn_get_free_channel(ISDN_USAGE_NET, lp->l2_proto,
							  ...
							  lp->pre_device,
						 lp->pre_channel)) < 0) {
							isdn_putlog(KERN_WARNING "isdn_net_find_icall: No channel for %s\n", lp->name);
							restore_flags(flags);
							return 0;
						}
						/* Setup dialstate. */
						...
						isdn_putlog(KERN_WARNING "isdn_net: %s: No phone number\n", lp->name);
					restore_flags(flags);
					return 0;
				} else {
					isdn_putlog(KERN_DEBUG "%s: call from %s -> %s accepted\n", lp->name, nr,
					       eaz);
	...
	if (!ematch || dev->net_verbose)
		isdn_putlog(KERN_INFO "isdn_net: call from %s -> %d %s ignored\n", nr, di, eaz);
	restore_flags(flags);
	return 0;
}
	...
	if ((!(lp->flags & ISDN_NET_CONNECTED)) && !lp->dialstate) {
		...
		if (lp->phone[1]) {
			...
			if ((chi = isdn_get_free_channel(ISDN_USAGE_NET, lp->l2_proto,
							 ...
							 lp->pre_device,
						 lp->pre_channel)) < 0) {
				isdn_putlog(KERN_WARNING "isdn_net_force_dial: No channel for %s\n", lp->name);
				restore_flags(flags);
				return -EAGAIN;
			}
			lp->dialstate = 1;
...
isdn_net_new(char *name, struct device *master)
{
	...
	if (isdn_net_findif(name)) {
		isdn_putlog(KERN_WARNING "isdn_net: interface %s already exists\n", name);
		return NULL;
	}
	if (!(netdev = (isdn_net_dev *) kmalloc(sizeof(isdn_net_dev), GFP_KERNEL))) {
		isdn_putlog(KERN_WARNING "isdn_net: Could not allocate net-device\n");
		return NULL;
	}
	...
	if (master) {
		...
		if (register_netdev(&netdev->dev) != 0) {
			isdn_putlog(KERN_WARNING "isdn_net: Could not register net-device\n");
			kfree(netdev);
			return NULL;
		}
	}
...
isdn_net_setcfg(isdn_net_ioctl_cfg * cfg)
{
	...
	if (p) {
		...
		if (i == ISDN_MAX_DRIVERS) {
			isdn_putlog(KERN_WARNING "isdn_net: No driver with selected features\n");
			return -ENODEV;
		}
		...
		if (cfg->p_encap == ISDN_NET_ENCAP_SYNCPPP) {
			...
			isdn_putlog(KERN_WARNING "%s: SyncPPP support not configured\n",
			       p->local.name);
			return -EINVAL;
#else
